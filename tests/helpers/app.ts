...
export function freshDataFile(): string {
  const f = path.join(
    os.tmpdir(),
    `innospace-bookings-test-${randomUUID()}.db`,
  );
  tmpFiles.push(f);
  return f;
}
 ...
 * lazy singleton starts clean. Call it, then import the db and/or route modules
 * you need: everything imported afterwards (until the next reset) shares that
 * one fresh DB.
 */
...
/** A valid dashboard session token (verifies under the fixed test AUTH_SECRET). */
